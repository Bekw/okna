...
class Api_Flexcell {
    ...
    function setMultipleRowDataDate($row, $dataset){
        //находим строку, в которой начинается scan##
        $first_cell = $this->getCellByValue($dataset.'##', false);
        //если элементов сканирования нет, то возвращаем false
        if ($first_cell == false)
            ...
            foreach ($row[$i] as $key => $value){
                if (isset($cell_field[$key]['col'])){
                    if (DateTime::createFromFormat('d.m.Y', $value) !== FALSE) {
                        ...
                        $this->worksheet->setCellValueByColumnAndRow($cell_field[$key]['col'] , $first_row + $i, PHPExcel_Shared_Date::PHPToExcel($date));
                    }else{
                        $this->worksheet->setCellValueByColumnAndRow($cell_field[$key]['col'] , $first_row + $i, $value);
                    }
                }
    ...
    function rowsAutoHeight(){
        foreach($this->worksheet->getRowDimensions() as $rd) {
            $rd->setRowHeight(-1);
        }
    }

    function save($dir,$name){
        ob_end_clean();
